...
function drawPath1() {
    ...
    function makeEdgeColorPalette() { return [C('red','239,68,68'), C('blue','96,165,250'), C('amber','245,158,11'), C('green','52,211,153'), C('edgeDim','160,160,200')]; }
    var edgeColorPalette = makeEdgeColorPalette();
    ...
    function animate() {
        ...
        hg.addColorStop(0, 'rgba(' + C('purple','167,139,250') + ',' + (0.18 * hp) + ')'); hg.addColorStop(1, 'transparent');
        ...
        ctx.fillStyle = 'rgba(' + C('purple','167,139,250') + ',' + (0.5 + hp * 0.35) + ')'; ctx.fill();

        // Header
        ctx.font = 'bold ' + FONT; ctx.textBaseline = 'middle'; ctx.textAlign = 'left';
        ctx.fillStyle = 'rgba(' + C('purple','167,139,250') + ',' + (0.75 + 0.1 * Math.sin(t * 1.5)) + ')';
        ctx.fillText(dirLabel, treeX, treeTopY);

        // ---- Trunk ----
        ctx.beginPath(); ctx.moveTo(nodeX, treeTopY + 8); ctx.lineTo(nodeX, lastY);
        ctx.strokeStyle = 'rgba(' + C('purple','167,139,250') + ',' + (0.1 + 0.05 * Math.sin(t * 1.2)) + ')'; ctx.lineWidth = 0.8; ctx.stroke();
        ...
        for (var si = 0; si < signals.length; si++) {
            ...
            ctx.fillStyle = 'rgba(' + C('purple','167,139,250') + ',' + (0.12 + sp * 0.15) + ')'; ctx.fill();
        }
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            grd.addColorStop(0, 'rgba(' + C('purple','167,139,250') + ',' + ((0.1 + boost * 0.15 + boostLevel * 0.08) * np) + ')');
            grd.addColorStop(1, 'transparent');
            ctx.beginPath(); ctx.arc(nX, nY, gr, 0, Math.PI * 2); ctx.fillStyle = grd; ctx.fill();
            ctx.beginPath(); ctx.arc(nX, nY, nr, 0, Math.PI * 2);
            ctx.fillStyle = 'rgba(' + C('purple','167,139,250') + ',' + (0.35 + np * 0.3 + boost * 0.2 + boostLevel * 0.1) + ')'; ctx.fill();

            // Hover bar
            if (boostLevel > 0) {
                ctx.fillStyle = 'rgba(' + C('purple','167,139,250') + ',' + (boostLevel * 0.06 * np) + ')';
            ...
            ctx.strokeStyle = 'rgba(' + C('purple','167,139,250') + ',' + (0.12 + boost * 0.1 + boostLevel * 0.06) + ')'; ctx.lineWidth = 0.6; ctx.stroke();
        ...
        if (hoveredFile < 0) {
            var hintA = 0.45 + 0.15 * Math.sin(t * 2);
            ctx.font = 'italic 10px JetBrains Mono';
            ctx.textAlign = 'left';
            ctx.fillStyle = 'rgba(' + C('mid','160,160,184') + ',' + hintA + ')';
...
function drawPath2() {
    ...
    function animate() {
        ...
        ctx.strokeStyle = 'rgba(' + C('blue','96,165,250') + ',0.5)'; ctx.lineWidth = 1.5; ctx.setLineDash([6, 4]); ctx.stroke(); ctx.setLineDash([]);
        ...
        for (var i = 0; i < cells.length; i++) {
            ...
            if (fires) {
                ...
                grd.addColorStop(0, 'rgba(' + C('green','52,211,153') + ',' + ((0.2 + excess * 0.15 + boost * 0.2) * pulse) + ')'); grd.addColorStop(1, 'transparent');
                ctx.beginPath(); ctx.arc(cx, cy, r * 3, 0, Math.PI * 2); ctx.fillStyle = grd; ctx.fill();
                ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2);
                ctx.fillStyle = 'rgba(' + C('green','52,211,153') + ',' + (0.4 + excess * 0.3 + pulse * 0.2 + boost * 0.15) + ')'; ctx.fill();
                ...
                for (var j = 0; j < cells.length; j++) {
                    ...
                    if (d < spX * 2.5) {
                        ...
                        ctx.strokeStyle = 'rgba(' + C('green','52,211,153') + ',' + ((1 - d / (spX * 2.5)) * 0.2 * pulse) + ')'; ctx.lineWidth = 0.6; ctx.stroke();
                    }
                }
            } else {
                var r = 1.8 + boost * 1.5;
                ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2);
                ctx.fillStyle = 'rgba(' + C('dim','92,92,116') + ',' + (0.15 + boost * 0.2) + ')'; ctx.fill();
            }
        }
...
function drawPath3() {
    ...
    function animate() {
        ...
        for (var i = 0; i < sessions.length - 1; i++) {
            ...
            ctx.strokeStyle = 'rgba(' + C('amber','245,158,11') + ',' + (0.3 + pulse * 0.2) + ')'; ctx.lineWidth = 1.5 + pulse * 0.8; ctx.stroke();
        }
        ...
        for (var i = 0; i < traceP.length; i++) {
            ...
            ctx.fillStyle = 'rgba(' + C('amber','245,158,11') + ',' + (0.3 + pulse * 0.25) + ')'; ctx.fill();
        }
        ...
        for (var si = 0; si < sessions.length; si++) {
            ...
            ctx.fillStyle = 'rgba(' + C('green','52,211,153') + ',' + (0.5 + 0.3 * Math.sin(t + si)) + ')';
            ctx.beginPath(); ctx.roundRect(bx, by, barW * k, barH, 2); ctx.fill();
            ctx.font = '8px JetBrains Mono';
            ctx.fillStyle = 'rgba(' + C('green','52,211,153') + ',' + (0.4 + k * 0.3) + ')'; ctx.fillText(Math.round(k * 100) + '%', sx, by + 14);
        }
...
function drawPath4() {
    ...
    function animate() {
        ...
        grd.addColorStop(0, 'rgba(' + C('green','52,211,153') + ',' + ((0.2 + cBoost * 0.2) * cPulse) + ')'); grd.addColorStop(1, 'transparent');
        ctx.beginPath(); ctx.arc(cx, cy, cr * 3.5, 0, Math.PI * 2); ctx.fillStyle = grd; ctx.fill();
        ctx.beginPath(); ctx.arc(cx, cy, cr, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(' + C('green','52,211,153') + ',' + (0.45 + cPulse * 0.3 + cBoost * 0.2) + ')'; ctx.fill();
